...
export default function LandingPageHeroImage() {
    return <section className="box-border not-content h-screen w-screen flex flex-col sm:flex-row flex-nowrap justify-center items-center gap-5 bg-hero_bg_sm sm:bg-hero_bg px-5 sm:px-0">
        ...
        <div className="relative z-10 flex flex-col flex-nowrap justify-center items-center gap-8 sm:justify-evenly sm:gap-80 sm:items-end w-full sm:w-1/4 h-1/2 sm:h-full py-5 sm:py-0 sm:pr-5">
            <img className="sm:absolute sm:top-20 sm:right-5 w-1/3 sm:w-3/4" src={ensnode_with_name.src} alt="ENSNode"/>
            <div className="sm:absolute sm:bottom-20 sm:right-5 flex flex-col flex-nowrap justify-start items-center sm:items-end gap-4">
                <h1 className="text-center sm:text-right text-white font-bold text-2xl sm:text-6xl sm:w-[600px]">The new<br /> multichain indexer for ENSv2</h1>
                <a href="/ensnode/" className="w-fit text-lg text-right text-white hover:cursor-pointer underline underline-offset-[4px] hover:underline-offset-[2px] transition-all duration-200">
                    Documentation &rarr;
                </a>
            </div>
        </div>
