...
const ImageSection = (props: InnovationSectionProps & ImageDisplayStyles) => (
    <div
        className={cc([
          "relative hidden flex-row justify-center items-center w-full h-2/3 xl:h-full xl:w-1/2 rounded-none bg-origin-border flex-shrink-0 max-w-2xl", props.displayStyles
        ])}
    >
      {props.normalImage ? (
          <img
...
export default function InnovationSection(props: InnovationSectionProps) {
  return (
      ...
      >
        <div
            className="flex flex-col xl:flex-row items-center justify-center xl:justify-between gap-10 sm:gap-5 xl:gap-20 max-w-[1216px]">
          {!props.isTextOnTheLeft && <ImageSection {...props} displayStyles="overflow-visible xl:flex"/>}
        ...
        {!props.isTextOnTheLeft && props.imageOnBottomForColumnLayout && <ImageSection {...props} displayStyles="sm:flex xl:hidden" />}
        {!props.mobileImageOnTop && (
          <div className="flex sm:hidden flex-row justify-center items-center w-full h-fit rounded-none bg-origin-border bg-center bg-no-repeat bg-contain flex-shrink-0">
            {props.designatedMobileImage ? props.designatedMobileImage : props.svgImage}
          </div>
