...
:::note[Image Size]
The ENSRainbow storage needed is from 1 MB to dozens GB depending on the namespace and label set.
## Production Setup

For ENS Subgraph compatibility use the `subgraph` namespace:
...
- **`SCHEMA_VERSION`**: Database schema version (e.g., `v3`)
- **`NAMESPACE`**: Data namespace identifier:
...
- **`LABEL_SET`**: Non-negative integer representing the dataset version within a namespace (e.g., `0`, `1`, `2`)
