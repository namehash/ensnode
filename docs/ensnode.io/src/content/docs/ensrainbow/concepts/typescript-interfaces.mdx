...
1. **Neither field** → accept any client labelset and use the latest version (default behaviour).
2. **Only `labelSetId`** → insist on a specific client labelset and use the latest version.
3. **Both fields** → insist on a specific client labelset and version, locking the client to an exact snapshot.
