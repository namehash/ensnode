...
The ENSRainbow Docker image is designed to be lightweight. The LevelDB database required for its operation is **not** baked into the image during the build process. Instead, the requested database is downloaded and managed at **runtime** by an `entrypoint.sh` script within the container. This process is triggered when you run the container with:
