...
The complete development guide is included below for reference, but the dedicated pages above provide more focused guidance for specific tasks.
:::
...
# Example: Download the 'ens-test-env' label set ID, label set version 0 (for the ens-test-env)
...
```
If `label_set_version` is provided in a heal request, then `label_set_id` must be also present. The server will validate them against the database's stored label set ID and highest label set version.
...
# Example: Download database schema 3, 'subgraph' label set ID, up to label set version 0
./scripts/download-prebuilt-database.sh 3 subgraph 0
```
This will download and verify `databases/3/subgraph-0.tgz` (relative to `OUT_DIR`). You can then extract this archive to your desired data directory (e.g., `./my-subgraph-data`) and use it with `pnpm run serve`.
...
- **Storage**: ~7-8 GB for a typical LevelDB database. Docker image size is about 1.5GB.
