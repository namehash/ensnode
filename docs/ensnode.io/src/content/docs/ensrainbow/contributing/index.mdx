...
2.  **Run the ENSRainbow container with test data configuration:**
    ...
    ```
    *   Adjust `SCHEMA_VERSION`, `NAMESPACE`, and `LABEL_SET` if the test data parameters differ. Typically, for the standard test data, `NAMESPACE` is `testenv` and `LABEL_SET` is `0`. The `SCHEMA_VERSION` should match the version of the database artifact you intend to use (e.g., `v3`).
