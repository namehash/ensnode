...
docker build -f apps/ensrainbow/Dockerfile.data \
  ...
  -t namehash/ensnode/ensrainbow-ens-test-env-data .

# Build the ENSRainbow image with the test data
docker build -f apps/ensrainbow/Dockerfile \
  --build-arg DATA_IMAGE_NAME=namehash/ensnode/ensrainbow-ens-test-env-data \
  -t namehash/ensnode/ensrainbow-ens-test-env .

# Run the ENSRainbow container with the test data
docker run -p 3223:3223 namehash/ensnode/ensrainbow-ens-test-env
```



This test environment setup is particularly useful for running ENS tests that require label healing capabilities without needing the full production dataset.
