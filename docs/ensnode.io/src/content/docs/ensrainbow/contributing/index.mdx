...
./scripts/download-ensrainbow-files.sh testenv 0
```
...
As an alternative to ingesting `.ensrainbow` files sequentially, you can download pre-built LevelDB database artifacts. These are `.tgz` archives containing a complete data directory. Use the `download-prebuilt-database.sh` script from `apps/ensrainbow/scripts/`.

The script downloads a tarball (`.tgz`) containing the database. The script requires `DB_SCHEMA_VERSION`, `LABEL_SET_ID`, and `LABEL_SET_VERSION` (highest label set version included in the artifact).
...
2.  **Run the ENSRainbow container with test data configuration:**
    ...
    ```
    *   Adjust `DB_SCHEMA_VERSION`, `LABEL_SET_ID`, and `LABEL_SET_VERSION` if the test data parameters differ. Typically, for the standard test data, `LABEL_SET_ID` is `testenv` and `LABEL_SET_VERSION` is `0`. The `DB_SCHEMA_VERSION` should match the version of the database artifact you intend to use (e.g., `v3`).
...
- **`DB_SCHEMA_VERSION`**: The database schema version (e.g., `v3`). This determines the format and structure of the database artifacts. Use the latest available version unless you have specific compatibility requirements.
