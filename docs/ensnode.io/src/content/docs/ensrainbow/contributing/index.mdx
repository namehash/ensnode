...
2.  **Run the ENSRainbow container with test data configuration:**
    ...
    ```
    *   Adjust `DB_SCHEMA_VERSION`, `LABEL_SET_ID`, and `LABEL_SET_VERSION` if the test data parameters differ. Typically, for the standard test data, `LABEL_SET_ID` is `testenv` and `LABEL_SET_VERSION` is `0`. The `DB_SCHEMA_VERSION` should match the version of the database archives you intend to use (e.g., `v3`).

This setup allows the `entrypoint.sh` script within the container to download the `testenv-0` database archive into the mounted volume on its first run. Subsequent runs will reuse the data from the volume.
...
When using ENSRainbow with Docker, the following environment variables control which database archive is downloaded and used:

- **`DB_SCHEMA_VERSION`**: The database schema version (e.g., `v3`). This determines the format and structure of the database archives. Use the latest available version unless you have specific compatibility requirements.
