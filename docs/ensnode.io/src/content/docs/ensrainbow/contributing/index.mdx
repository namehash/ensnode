...
ENSRainbow ingests data from `.ensrainbow` files. These files contain labelhash-label pairs for a specific `LABEL_SET_ID` and `LABEL_SET_VERSION`. You can download these files using the `download-ensrainbow-files.sh` script located in `apps/ensrainbow/`.
...
# Example: Download 'testenv' label set ID, label set version 0 (for test environment)
...
- Place the files in `./labelsets/<LABEL_SET_ID>-<LABEL_SET_VERSION>.ensrainbow` and `./THE_GRAPH_LICENSE.txt` respectively (relative to where `OUT_DIR` is set, default is current dir).
...
The script requires `SCHEMA_VERSION`, `LABEL_SET_ID`, and `LABEL_SET_VERSION` (highest label set version included in the artifact).
...
2.  **Run the ENSRainbow container with test data configuration:**
    You'll need to mount a volume for persistent storage and set the `SCHEMA_VERSION`, `LABEL_SET_ID`, and `LABEL_SET_VERSION` environment variables to point to the test data.
    ```bash
    ...
    ```
    *   Adjust `SCHEMA_VERSION`, `LABEL_SET_ID`, and `LABEL_SET_VERSION` if the test data parameters differ. Typically, for the standard test data, `LABEL_SET_ID` is `testenv` and `LABEL_SET_VERSION` is `0`. The `SCHEMA_VERSION` should match the version of the database artifact you intend to use (e.g., `v3`).
...
- **`LABEL_SET_ID`**: The data namespace identifier that categorizes the type of ENS data:
...
- **`LABEL_SET_VERSION`**: A non-negative integer representing the incremental dataset version within a label set ID. Label sets are sequential:
...
SCHEMA_VERSION=v3 LABEL_SET_ID=subgraph LABEL_SET_VERSION=0

# Test environment data
SCHEMA_VERSION=v3 LABEL_SET_ID=testenv LABEL_SET_VERSION=0

# Extended discovery data
SCHEMA_VERSION=v3 LABEL_SET_ID=searchlight LABEL_SET_VERSION=0
```
...
1.  **On First Run (with an empty volume):**
    *   The script requires `SCHEMA_VERSION`, `LABEL_SET_ID`, and `LABEL_SET_VERSION` environment variables to be set.
...
Adjust `SCHEMA_VERSION`, `LABEL_SET_ID`, and `LABEL_SET_VERSION` as needed for the initial download.
...
**For `subgraph` label set ID (main dataset):**
This command converts a SQL dump file (`ens_names.sql.gz`) into an `.ensrainbow` file for the `subgraph` label set ID and label set version 0.
```bash
# Assuming ens_names.sql.gz contains the primary dataset
time pnpm run convert --input-file ens_names.sql.gz --output-file subgraph_0.ensrainbow --label_set_id subgraph --label_set_version 0
```

**For `discoveryA` label set ID (initially empty for discovered labels):**
This creates an empty `.ensrainbow` file for the `discoveryA` label set ID, which might be used for labels discovered dynamically.
```bash
touch empty.sql
gzip empty.sql
time pnpm run convert --input-file empty.sql.gz --output-file discoveryA_0.ensrainbow --label_set_id discoveryA --label_set_version 0
```

**For `testenv` label set ID (for testing):**
This converts a test dataset SQL dump into an `.ensrainbow` file for the `testenv` label set ID.
```bash
time pnpm run convert --input-file test/fixtures/ens_test_env_names.sql.gz --output-file testenv_0.ensrainbow --label_set_id testenv --label_set_version 0
```
