...
./scripts/download-ensrainbow-files.sh subgraph 0   # classic ENS Subgraph dataset, version 0
```
...
## 2  (Optional) Convert a legacy SQL dump

If you have a gzipped rainbow table (`ens_names.sql.gz`) you can convert it:

```bash title="Convert legacy SQL data"
pnpm run convert --input-file path/to/ens_names.sql.gz --output-file subgraph-0.ensrainbow
```

## 3  Ingest into LevelDB
