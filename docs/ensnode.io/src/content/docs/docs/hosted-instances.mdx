...
export const subgraphCompatibilityFull = `Full Subgraph Compatibility, both, on API-level and data-level. ENSNode can be used as a drop-in replacement for the ENS Subgraph, with guarantees that all data returned by ENSNode will be fully aligned with the ENS Subgraph.`;
...
export const omnigraphUnavaialble = `Not activated due to this instance's focus on maximum compatibility with the ENS Subgraph.`;
