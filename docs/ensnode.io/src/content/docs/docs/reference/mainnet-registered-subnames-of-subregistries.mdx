...
This reference discusses the situation in which a name is registered on multiple chains (ex. `example.base.eth` being registered on both the mainnet Regsitry and Basenames [**Shadow Registry**](/docs/reference/terminology#shadow-registry)).
