...
The legacy ENS Subgraph specifies that labels containing certain UTF-8 characters are 'unindexable'.

The `subgraph-unindexable` UTF-8 characters are: `\0` (NULL), `[`, `]`, and `.` (period).

- `subgraph-indexable`: the Literal Label in question does NOT contain any of the prohibited UTF-8 characters
- `subgraph-unindexable`: the Literal Label in question contains some of the prohibited UTF-8 characters

### Subgraph Interpreted Label
A **Subgraph Interpreted Label** is a **Literal Label** that is either:
- (if `subgraph-indexable`): a Literal Label, of unknown normalization status, guaranteed to not contain any of the `subgraph-unindexable` UTF-8 characters
