...
The ENSv2 upgrade to the ENS protocol is coming **Summer 2026**! Your app, regardless of how it interacts with names, needs to be updated to avoid being left behind.<br/>[Learn more about ENSv2 Readiness](/docs/integrate/why-ensnode/ensv2-readiness)
</Aside>
...
export function DomainCard({ name }: { name: InterpretedName }) {
  ...
  return (
    <div>
      ...
      <p>Ethereum address: {domain.resolve?.profile?.addresses?.ethereum}</p>
      <p>Description: {domain.resolve?.profile?.description}</p>
    </div>
  );
}
```
