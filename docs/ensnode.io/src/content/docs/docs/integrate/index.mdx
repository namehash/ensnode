...
Planning to self-host? We're building [**`ensdb-cli` & ENSDb snapshots**](/docs/integrate/ensdb-cli) so you can stand up a fresh, nearly-current ENSDb in minutes instead of waiting days and spending big on RPC credits to complete a full historical backfill.
:::
