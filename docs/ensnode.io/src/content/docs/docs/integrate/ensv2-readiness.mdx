...
1. **ENS resolution** — RPC calls with CCIP-read support for offchain data (e.g. via [viem](/docs/reference/subgraph-legacy/with-viem)) to perform forward or reverse ENS resolution.
...
- The Subgraph gives you queryable indexed ENS data but has many limitations, including:
