...
<UpgradeToOmnigraphRequiredTable>
  <ENSSubgraphDependentApp toENSNodeSubgraphCompatibleEndpoint={true} toOmnigraphAPI={true}>
  <a href="/docs/integrate/integration-options/omnigraph-mcp" class="omnigraph-required-table-content"><strong>ENSNode Omnigraph MCP</strong></a> — first-party, built into every ENSNode instance at <code class="bg-[var(--sl-color-bg-inline-code)] text-xs px-1.5 py-0.5 -my-0.5">/api/mcp</code>. Runs queries directly against the ENS Omnigraph API.
  </ENSSubgraphDependentApp>
