...
3. In chat, ask ENS questions in plain language — e.g. _"Who owns vitalik.eth?"_ or _"List domains owned by 0x…"_ — and let the agent call `omnigraph_query`. Use the server key `"ENS"` in your config so the MCP entry is easy to spot; the agent picks the right `exampleId` or writes a custom query for your question.
