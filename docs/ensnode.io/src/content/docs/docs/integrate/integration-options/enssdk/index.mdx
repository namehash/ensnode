...
<Code code={`npm install enssdk@${snapshot.sdkVersion}\nnpm install -D tsx typescript @types/node`} lang="sh" />
...
```json title="tsconfig.json" ins={11-17}
{
  "compilerOptions": {
    "target": "ES2022",
    "module": "ESNext",
...
async function main() {
  ...
  console.log(`Name: ${domain.canonical ? domain.canonical.name.beautified : "<unnamed>"}`);
...
async function main() {
  ...
  for (const { node } of domain.subdomains?.edges ?? []) {
    const subName = node.canonical ? node.canonical.name.beautified : "<unnamed>";
