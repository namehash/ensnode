...
async function main() {
  ...
  console.table(
    (domain.subdomains?.edges ?? []).map(({ node }) => ({
      name: node.name ? beautifyInterpretedName(node.name) : "<unnamed>",
...
`FragmentOf<typeof DomainFragment>` is the opaque type for any selection that includes `...DomainFragment` — `formatDomain` accepts any of them. `readFragment(DomainFragment, data)` unwraps that opaque type to the typed fields you declared.
