...
Every ENSNode instance also serves the Omnigraph over the Model Context Protocol at `/api/mcp`. Point an MCP client (Cursor, Claude Desktop, agents) at the URL and run Omnigraph queries directly — first-party, read-only, nothing to install.

<LinkCard
  title="ENS Omnigraph MCP Documentation"
