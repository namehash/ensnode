...
- **Permissions** — the ENSv2 roles model for who can do what to a name.

## Canonical fields: query names without branching

Every Domain reachable from the canonical root has its **canonical fields materialized** — `canonical_name`, `canonical_path`, `canonical_node`, and `canonical_depth` — across both ENSv1 and ENSv2. That means you can look a name up by `canonical_name = 'vitalik.eth'`, order by `canonical_depth`, or walk a name's path **without branching on protocol version** and without re-deriving the namegraph yourself.
