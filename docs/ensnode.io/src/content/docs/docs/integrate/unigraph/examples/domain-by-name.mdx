...
import UnigraphStaticExampleWrapper from "@components/molecules/unigraph-static-example/index.astro";
import StaticExampleCodeSection from "@components/molecules/omnigraph-static-example/StaticExampleCodeSection.astro";
import StaticExampleOutputSection from "@components/molecules/omnigraph-static-example/StaticExampleOutputSection.astro";
import { stringifyJsonForDocs } from "@lib/examples/omnigraph/docs-utils";
...
export const exampleSQLSnippet = `SELECT * FROM ensindexer_0.domains\nWHERE canonical_name = 'vitalik.eth';`
...
<Tabs syncKey="unigraph-client">
  <TabItem label="SQL" icon="database">
    ...
    <StaticExampleCodeSection badge="SQL" lang="sql" code={exampleSQLSnippet} />

    <StaticExampleOutputSection
      uid="unigraph-domain-by-name-result-sql"
      responseJson={stringifyJsonForDocs(unigraphSqlResultJson)}
      heading="Output"
    />
