...
The `SELECT` still returns `canonical_name`; only the `LIKE` / `ORDER BY` use the prefix. For exact matches, use `canonical_name` directly (`canonical_name = 'vitalik.eth'`).
:::
