...
The Indexing Status API supports a `maxRealtimeDistance` field in response object
that enables this verification.

#### How it works

The `maxRealtimeDistance` field holds a duration value in seconds,
representing the max distance between the now and the latest indexed block of each chain and the "tip" of all indexed chains.
