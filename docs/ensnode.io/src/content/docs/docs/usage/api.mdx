...
<LinkCard
  href="/docs/concepts/what-is-the-ens-subgraph/"
...
which supports an optional `requestedMaxWorstCaseDistance` parameter (for which
default value is `AMIREALTIME_DEFAULT_MAX_WORST_CASE_DISTANCE`).

The `requestedMaxWorstCaseDistance` parameter accepts a duration value in seconds,
