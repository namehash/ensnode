...
Therefore, always use the node of a name (calculated by the namehash of the name) as the stable identifier when querying the ENSNode API. The node of a name is immutable across time and works for all names, even if they are unknown, unnormalized, or subgraph-invalid.
...
ENSNode's handling of unnormalized labels is controlled by the `REPLACE_UNNORMALIZED` configuration option:
- `REPLACE_UNNORMALIZED=false` allows unnormalized labels to be returned (required for full ENS Subgraph compatibility)
- `REPLACE_UNNORMALIZED=true` (default) encodes unnormalized labels as labelhashes, improving security

For more details, see [Subgraph Compatibility](/docs/reference/subgraph-compatibility/#replace_unnormalized).
:::

### When `REPLACE_UNNORMALIZED=false`
...
### When `REPLACE_UNNORMALIZED=true`

When ENSNode is configured to replace unnormalized labels with encoded labelHashes, it helps you avoid edge cases related to null bytes, full-stop characters (periods), or exotic unicode characters. When names are returned from any of the ENSNode APIs, including the Subgraph-compatible GraphQL API, the only names will either be a) [normalized](https://ensnode.io/docs/reference/terminology/#name-node-namehash) or b) unnormalized in [encoded labelHash form](https://ensnode.io/docs/reference/terminology/#rendering-unknown-labels).
...
## ENS Subgraph 'Valid' and 'Invalid' Labels

When `REPLACE_UNNORMALIZED=false`, ENSNode may sometimes represent a label as an unknown label if that label is not "valid" according to the legacy ENS Subgraph logic. This is an important consideration when working with the ENSNode API when `REPLACE_UNNORMALIZED=false`.

While many characters are not supported by ENSIP-15, only the following four characters are specifically classified as subgraph-invalid due to indexing concerns:
...
When a subgraph-invalid label is encountered, ENSNode represents it as an "unknown label" in the format `[{labelhash}]`, where `{labelhash}` is the labelhash of the unknown label. This representation creates an interesting edge case that must be handled carefully:

Consider an unnormalized label that literally looks like `[24695ee963d29f0f52edfdea1e830d2fcfc9052d5ba70b194bddd0afbbc89765]`. Because this label contains square brackets (subgraph-invalid characters), it will be represented as the unknown label:
`[80968d00b78a91f47b233eaa213576293d16dadcbbdceb257bca94b08451ba7f]`

Therefore, this represents the subgraph-invalid label as an "unknown label", encoding the labelhash of the original unnormalized label (including its square brackets) in square brackets. This demonstrates why square brackets are considered subgraph-invalid - they create ambiguity between literal labels and the representation of unknown labels.

When ENSNode encounters an subgraph-invalid label, it will represent it as an unknown label even if the actual label data is available. This is a safety mechanism to ensure data integrity and consistent behavior across the system.

For more detailed information about subgraph-invalid labels and their handling, please refer to the [ENSNode SDK implementation](https://github.com/namehash/ensnode/blob/main/packages/@ensnode/ensnode-sdk/src/ens/subgraph-label-valid.ts).
