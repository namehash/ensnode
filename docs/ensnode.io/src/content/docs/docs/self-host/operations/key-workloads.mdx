...
:::danger[ENSIndexer always starts from the backfill]
Even if your ENSIndexer instance has been indexing live data, when it restarts, it will always [start the "crash recovery" process](/docs/services/ensindexer/concepts/startup-sequence#ensdb-writer-schema-migrations), which leads back to the backfill. A huge downside of this is that ENSIndexer will drop indexes in the [ENSIndexer Schema](/docs/services/ensdb/concepts/glossary#ensindexer-schema) and after completing the backfill in a short while, it will have to rebuilt the indexes again. Depending on the selected [ENS Namespace](/docs/hosted-instances#ens-namespaces) and activated [ENSNode Plugins](/docs/hosted-instances#ensnode-plugins), this can take from a few minutes to up to several hours.
:::
