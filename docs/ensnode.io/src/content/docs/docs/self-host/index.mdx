...
Self-hosting today means standing up a fresh ENSNode instance from zero and waiting on an initial backfill - including the RPC bill that comes with it. We're building **[ENSDb snapshots and an `ensdb-cli`](/docs/integrate/ensdb-cli)** to flatten that ramp.
