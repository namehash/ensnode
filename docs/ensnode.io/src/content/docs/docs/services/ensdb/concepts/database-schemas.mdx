...
| `canonical_label_hash_path`  | `text[]`      | yes      | Materialized Canonical LabelHashPath, `NULL` iff `canonical = false`. In traversal-order (i.e. `[labelhash("eth"), labelhash("vitalik")]`). Maintained by `canonicality-db-helpers.ts`. |
| `canonical_path`             | `text[]`      | yes      | Materialized Canonical Domain Path, `NULL` iff `canonical = false`. In traversal-order (i.e. `["eth"`'s `DomainId`, `"vitalik"`'s `DomainId]`). Maintained by `canonicality-db-helpers.ts`. |
