...
ENSIndexer is designed to be extensible with plugins that can implement custom indexing logic and data models. This allows you to tailor ENSIndexer to your specific use case and ENS data needs.

ENSIndexer will only active the plugins you specify in the `PLUGINS` env variable, so you can choose to only activate the plugins that are relevant to your use case. The `PLUGINS` env variable accepts a comma-separated list of plugin names to be activated. For example, if you want to activate the `ensv2` and `protocol-acceleration` plugins, you can set `PLUGINS=ensv2,protocol-acceleration`.

## Plugin Architecture

The ENSIndexer plugin is a standalone TypeScript module that can be developed, tested, and deployed independently. Plugins can define their own data models, and indexing logic. It must include two main files:
- `event-handlers.ts`: This file defines the event handlers for the plugin, which are functions that will be called when specific ENS events are indexed. The event handlers contain the logic for how to process the indexed data and store it in the ENSIndexer Schema.
...
The `event-handlers.ts` file has to export a function via default export. This function will be only called by ENSIndexer runtime when the plugin is activated. We leverage this approach to conditionally execute `addOnchainEventListener` calls for active plugins only. This way, ENSIndexer learns which specific onchain events it need to listen to, and what indexing logic to execute for each indexed event, based on the event handlers from the active plugins.
