...
Each instance of ENSIndexer maintains its own ENSIndexer Schema instance in ENSDb. The same ENSIndexer instance will always use the same ENSIndexer Schema, even after restarts. However, if you need to update ENSIndexer config, or deploy a new version of ENSIndexer, you will have to update the `ENSINDEXER_SCHEMA_NAME` env variable to point to a new schema, as otherwise, the ENSIndexer instance will detect that the ENSIndexer Schema was already owned by an ENSIndexer instance with different config / indexing logic and will refuse to start.
...
- The solution is that you need to manually delete all the old `ENSINDEXER_SCHEMA_NAME` in your ENSDb instance you no longer use.

### Dropping orphaned ENSIndexer Schemas

If your ENSDb instance have orphaned ENSIndexer Schemas that you no longer use, you can drop them with a SQL command like the following:
