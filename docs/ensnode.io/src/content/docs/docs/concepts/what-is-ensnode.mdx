...
When ENSIndexer is run, the configs for all of the active plugins (those selected by the user) are merged and ponder runs in `omnichain` (soon: `multichain`) mode to produce the resulting index. The relevant event handlers are attached in each plugin's `event-handlers.ts` which conditionally executes if the plugin is activated in the ENSIndexerConfig.
