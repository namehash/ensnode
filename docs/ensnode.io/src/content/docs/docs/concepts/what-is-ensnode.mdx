...
- `event-handlers.ts` gathers all event handlers executed during Ponder indexing process for the plugin. See `apps/ensindexer/src/plugins/basenames/event-handlers.ts` and `apps/ensindexer/src/plugins/event-lineanames/handlers.ts` for examples.
