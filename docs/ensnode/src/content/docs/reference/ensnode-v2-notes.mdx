...
ponder multichain-indexes all on-chain state and shoves it into the postgres database. our ensnode api server talks to ponder's postgres tables via shared drizzle schema.
