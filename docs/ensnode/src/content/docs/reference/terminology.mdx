...
For the _normalized_ name composed of a _known_ labels `vitalik.eth`, the **node** is `0xee6c4522aab0003e8d14cd40a6af439055fd2577951148c14b6cea9a53475835`.
...
In the ENS `Registry` contract, only the **name**'s **node** is registered on-chain. Because of this there is no guarantee that a registered **node** is composed of _normalized_ labels or that those labels are _known_. That is, by default, the human-readable **labels** that make up a **name** are not available (either on-chain or off-chain). In some cases the human-readable _known_ **labels** that make up a name are emitted by contracts (i.e. the `ETHRegistrarController` or the `NameWrapper`), and in other cases the human-readable **label** for a given **labelhash** can be determined via rainbow table lookups and other strategies.
