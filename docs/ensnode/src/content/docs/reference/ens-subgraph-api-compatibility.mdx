...
1. Demonstrating 1:1 reproduction of the ENS Subgraph's logic & data
    - the ENS Subgraph has been adopted by the community as the source of truth regarding ENS protocol data, and confidence in ENSNode's implementation is demanded
2. Plug-and-Play compatibility for those that wish to host their own ENS indexer
    - deploying ENSNode is faster and easier than running an ENS Subgraph instance and likely more reliable at scale
...
The following features of the subgraph graphql api are explicitly unsupported and are not planned to be supported.
...
- [`getDecodedName`](https://github.com/ensdomains/ensjs/blob/main/packages/ensjs/src/functions/subgraph/getDecodedName.ts) — Gets the full name for a name with unknown labels from the subgraph
  - Attempts to heal any encoded labels in a provided name using the subgraph
  - If name is fully decoded, returns
  - Splits name into labels
  - For encoded labels, finds domains by id
  - Queries domain by namehash
  - In an ideal implementation, this would be a single domain query that handles healing internally
