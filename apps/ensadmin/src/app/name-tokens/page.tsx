    ...
    <div className="p-6">
      <strong>
        ...
        <small>
          (accurate as of{" "}
          <AbsoluteTime
            timestamp={nameTokens.data.accurateAsOf}
            options={{
              year: "numeric",
              month: "short",
              day: "numeric",
      ...
      <details>
        ...
        <ul>
          {nameTokens.data.nameTokens.map((nameToken) => (
            <li key={serializeAssetId(nameToken.token.domainAsset)}>
              <pre>
                {JSON.stringify(nameToken, (_, v) => (typeof v === "bigint" ? v.toString() : v), 2)}
              </pre>
            </li>
          ))}
        </ul>
