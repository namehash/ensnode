...
const mockIndexerData = mockDataJson as Record<string, {ensIndexerConfig: SerializedENSIndexerPublicConfig, indexingStatus: SerializedENSIndexerOverallIndexingStatus}>;
...
export default function MockRegistrationsPage() {
    ...
    const props: RecentRegistrationsProps = useMemo(() => {
        switch (selectedVariant) {
            ...
            default:
                try {
                    const config = deserializeENSIndexerPublicConfig(mockIndexerData[selectedVariant].ensIndexerConfig);
                    const indexingStatus = deserializeENSIndexerIndexingStatus(mockIndexerData[selectedVariant].indexingStatus);
    ...
    return <section className="flex flex-col gap-6 p-6 max-sm:p-4">
        <Card>
            <CardHeader>
