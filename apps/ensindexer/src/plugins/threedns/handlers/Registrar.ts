...
export default function ({ ownedName, namespace }: PonderENSPluginHandlerArgs<"">) {
  ...
  ponder.on(namespace("ThreeDNSRegControl:NameRegistered"), async ({ context, event }) => {
  ...
  ponder.on(namespace("ThreeDNSRegControl:NameRenewed"), async ({ context, event }) => {
  ...
  ponder.on(namespace("ThreeDNSRegControl:Transfer"), async ({ context, event }) => {
