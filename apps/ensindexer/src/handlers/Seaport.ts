...
async function handleOffer(
  ...
  const buyer = offerer as Address;
  const seller = recipient as Address;
  ...
  // Get currency info
  const currencyId = await upsertCurrency(context, payment.token as Address);
...
async function handleListing(
  ...
  const seller = offerer as Address;
  const buyer = recipient as Address;
