...
if (config.plugins.includes(PluginName.Subgraph) && config.plugins.includes(PluginName.ENSv2)) {
  console.warn(
    `Both the '${PluginName.Subgraph}' and '${PluginName.ENSv2} plugins are enabled. This results in the availability of both the legacy Subgrpah-Compatible GraphQL API (/subgraph) _and_ ENSNode's GraphQL API (/api/graphql), and comes with an associated increase in indexing time. If your intent is to have both APIs available in parallel, excellent, otherwise you may benefit from only enabling the plugin for the API you plan to use.`,
  );
}
