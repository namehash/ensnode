...
import type { Span, Trace, TraceNode } from "@ensnode/ensnode-sdk";
...
export class TracingSpan {
  ...
  constructor(
    ...
    this.id = `span-${++TracingSpan.counter}-${Date.now()}`;
...
export class TracingContextManager {
   ...
   */
  async runWithTrace<T>(fn: () => Promise<T>): Promise<{ result: T; trace: Trace }> {
  ...
  async withSpan<T>(
    ...
    const span = new TracingSpan(name, context.traceId, context.activeSpan?.id, attributes);
   ...
   */
  private buildTrace(spans: TracingSpan[]): Trace {
    const idToNode = new Map<string, TraceNode>();
    const roots: TraceNode[] = [];

    // Convert tracing spans to spans and create nodes
    for (const span of spans) {
      const _span = span.toSpan();
      idToNode.set(_span.id, { ..._span, children: [] });
    }
