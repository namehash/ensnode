...
const logger = makeLogger("referrer-leaderboard-cache-v1");
...
/**
 * Cache for the V1 referrer leaderboard API.
 */
export const referrerLeaderboardCacheV1 = new SWRCache({
  fn: async () => {
    ...
    try {
      ...
      logger.info(
        `Successfully built referrer leaderboard (V1) with ${result.referrers.size} referrers from indexed data`,
      );
      return result;
    } catch (error) {
      logger.error({ error }, "Failed to build referrer leaderboard (V1)");
      throw error;
    }
  },
