...
const _handleNewOwner =
  ...
  async ({
    ...
    if (domain) {
      ...
      console.log("New domain created", domain);
...
function initRegistryHandlers() {
  // setup on registry
  ponder.on(ns("Registry:setup"), async ({ context }) => {
  ...
  ponder.on(ns("Registry:NewTTL"), _handleNewTTL);
  ponder.on(ns("Registry:Transfer"), _handleTransfer);
}
