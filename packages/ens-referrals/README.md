...
- [`v1`](src/v1) is the actively supported version that reflects the current ENS Referrals program rules and awards.
- The root [`@namehash/ens-referrals`](src/client.ts) import is kept only for backwards compatibility with the ENS Holiday Awards edition.
