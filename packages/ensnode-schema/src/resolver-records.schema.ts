/**
 * Schema Definitions for optional tracking of Resolver records (both keys _and_ values).
 *
 ...
 *
 * During resolution, a single ENSIP-10 'active' Resolver contract for a Node is identified, and it is
 * only that Resolver that is queried in order to resolve the 'active' records for the given Node.
 *
 ...
 *
 * In this file, we extend the subgraph's Resolver _entity_ with the following:
 * Resolver (the Resolver-Node relationship)
 *   + has one `name` record for a Node (see ENSIP-3)
 *   + has many `addressRecords` for a Node (by coinType) (see ENSIP-9)
 *     - ResolverAddressRecord indexes `coinType` and `address`
 *   + has many `textRecords` for a Node (by key) (see ENSIP-5)
 *     - ResolverTextRecord indexes `key` and `value`
 *
 * This results in the following query patterns for the autogenerated GraphQL APIs:
 * @example
 * ```gql
 * # NOTE: generate the Resolver entity ID as:
 ...
 * query RecordsForNodeOnResolver($id: ID!) {
 *   resolver(id: $id) {
 *     addr
 *     name
 *     addressRecords {
 *       totalCount
 *       items {
 *         coinType     # see ENSIP-9, ENSIP-11
 *         address      # see ENSIP-9, ENSIP-11
 *       }
 *     }
 *     textRecords {
 *       totalCount
 *       items {
 *         key          # see ENSIP-5, ENSIP-12, ENSIP-18
 *         value        # see ENSIP-5, ENSIP-12, ENSIP-18
 *       }
 *     }
 *   }
 * }
 *```
 *
 * NOTE: The addition of these record keys and values does NOT allow the caller to confidently
 * resolve records for names: a direct lookup of a Name's Resolver and retrieval of its values
 * is NOT ENSIP-10 NOR CCIP-Read compatible.
 */
