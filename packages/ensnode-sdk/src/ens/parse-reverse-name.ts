...
/**
 * Parses an address (hex sans prefix) into an Address.
 *
 * @throws if address is invalid
 */
const parseAddressLabel = (addressLabel: string): Address => getAddress(`0x${addressLabel}`);

/**
 * Parses a coinType (hex sans prefix) into an EVMCoinType.
 *
 * @throws if coinType is invalid
 */
const parseCoinTypeLabel = (coinTypeLabel: string): CoinType => {
