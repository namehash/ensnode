...
describe("Chain Indexing Status Snapshot Schema", () => {
  ...
  describe("ChainIndexingStatusSnapshotQueued", () => {
    ...
    it("won't parse if the config.startBlock is after the config.endBlock", () => {
      ...
      // assert
      expect(notParsed).toMatch(/`config.startBlock` must be before or same as `config.endBlock`/i);
    });
  });
  ...
  describe("ChainIndexingStatusSnapshotCompleted", () => {
    ...
    it("won't parse if the latestIndexedBlock is after the config.endBlock", () => {
      ...
      // assert
      expect(notParsed).toMatch(/`latestIndexedBlock` must be before or same as `config.endBlock`/);
    });
  });
});
