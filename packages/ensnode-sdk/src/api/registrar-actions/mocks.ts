import {
  ...
  type SerializedNamedRegistrarAction,
} from "../..";
import { RegistrarActionsResponseCodes } from "./response";
import type {
  SerializedRegistrarActionsResponseError,
  SerializedRegistrarActionsResponseOk,
} from "./serialized-response";

export const validNamedRegistrarActionNormalizedWithReferral = {
  action: {
...
export const validNamedRegistrarActionEncodedLabelHash = {
  action: {
...
export const validResponseError = {
  responseCode: RegistrarActionsResponseCodes.Error,
  error: {
    message: "Registrar Actions API is not available",
    details: `The cached omnichain indexing status of the Connected ENSIndexer must be ${getOmnichainIndexingStatusIdFinal(ChainIndexingConfigTypeIds.Definite)}.`,
  },
