...
# Plan/region are hardcoded (not secrets) — the bare-metal SKU + a space-separated region fallback
...
# runner dies and never tears it down. Keep comfortably above a full index for headroom — 25h leaves
# margin across a couple of ~12h Blacksmith runner reclaims (the detached producer survives them and
# re-runs re-attach).
export SELF_DESTRUCT_HOURS="${SELF_DESTRUCT_HOURS:-25}"
